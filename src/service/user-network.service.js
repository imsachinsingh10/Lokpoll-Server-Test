...
export class UserNetworkService {
    ...
    async getCoinLogs(userId) {
        const q = `select cal.*, ca.name activityName from ${table.coin_activity_log} cal
                    join ${table.coin_activity} ca on ca.activity = cal.activity
                    where cal.userId = ${userId} order by id desc;`
