...
export class PostController {
    ...
    async creditCoinsOnRePost(req, postId, userId) {
        const reqBody = req.body;
        const files = req.files;
    ...
    async shareInternally(req) {
        ...
        if (!_.isEmpty(postMedia)) {
			 ...
			 postMedia = postMedia.map(({postId, commentId, type, url, thumbnailUrl}) => {
			   return {postId, commentId, type, url, thumbnailUrl};
			});
